    ...
    change: function() {
      ...
      Array.prototype.forEach.call(files, function(file) {
        if (!isTypeValid(file, self.accept)) {
          // 通知文件类型不合要求
          self.fire('error', TYPE_INVALID, file);
        } else if (!isSizeValid(file, self.maxSize)) {
          // 通知文件尺寸不合要求
          self.fire('error', SIZE_EXCEED, file);
        } else if (self.fire('change', file) !== false) {
  ...
  function isTypeValid(file, accept) {
    ...
    if ((type = getFileType(file))) {
      type = type.split(/[\.\/]/);
      isValid = accept.some(function(accept) {
        return (!type[0] || !accept[0] || type[0] === accept[0]) &&
