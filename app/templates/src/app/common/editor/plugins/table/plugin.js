  ...
  tinymce.PluginManager.add('table', function(editor) {
    ...
    function unApplyAlign(elm) {
      tinymce.each('left center right'.split(' '), function(name) {
        editor.formatter.remove('align' + name, {}, elm);
      });
    }

    function unApplyVAlign(elm) {
      tinymce.each('top middle bottom'.split(' '), function(name) {
    ...
    function tableDialog() {
      var dom = editor.dom,
        tableElm, colsCtrl, rowsCtrl, data;
        ...
        onsubmit: function() {
          ...
          editor.undoManager.transact(function() {
            ...
            if (!captionElm && data.caption) {
              ...
              captionElm.innerHTML = !tinymce.Env.ie ? '<br data-mce-bogus="1"/>' : '\u00a0';
    ...
    function cellDialog() {
      ...
      editor.windowManager.open({
        ...
        onsubmit: function() {
          ...
          editor.undoManager.transact(function() {
            tinymce.each(cells, function(cellElm) {
    ...
    function rowDialog() {
      var dom = editor.dom,
        tableElm, cellElm, rowElm, data, rows = [];

      tableElm = editor.dom.getParent(editor.selection.getStart(), 'table');
      cellElm = editor.dom.getParent(editor.selection.getStart(), 'td,th');

      tinymce.each(tableElm.rows, function(row) {
      ...
      editor.windowManager.open({
        ...
        onsubmit: function() {
          var data = this.toJSON(),
            tableElm, oldParentElm, parentElm;

          editor.undoManager.transact(function() {
            var toType = data.type;

            tinymce.each(rows, function(rowElm) {
    ...
    function insertTable(cols, rows) {
      ...
      html = '<div class="p-table"><table id="__mce"><tbody>';
    ...
    var menuItems = [];
    tinymce.each('inserttable tableprops deletetable | cell row column'.split(' '), function(name) {
      if (name == '|') {
        menuItems.push({
          text: '-'
        });
