...
return function(editor, table) {
  ...
  function buildGrid() {
    ...
    tinymce.each(['thead', 'tbody', 'tfoot'], function(part) {
      var rows = dom.select('> ' + part + ' tr', table);

      tinymce.each(rows, function(tr, y) {
        y += startY;

        tinymce.each(dom.select('> td, > th', tr), function(td, x) {
  ...
  function getSelectedRows() {
    ...
    tinymce.each(table.rows, function(row) {
      tinymce.each(row.cells, function(cell) {
  ...
  function cloneCell(cell) {
    ...
    tinymce.walk(cell, function(node) {
      ...
      if (node.nodeType == 3) {
        tinymce.each(dom.getParents(node.parentNode, null, cell).reverse(), function(node) {
        ...
        if (curNode) {
          curNode.innerHTML = tinymce.Env.ie ? '&nbsp;' : '<br data-mce-bogus="1" />';
        }
  ...
  function cleanup() {
    ...
    // Empty rows
    tinymce.each(dom.select('tr', table), function(tr) {
    ...
    tinymce.each(dom.select('thead,tbody,tfoot', table), function(part) {
  ...
  function split() {
    tinymce.each(grid, function(row, y) {
      tinymce.each(row, function(cell, x) {
      ...
      for (y = startY; y <= endY; y++) {
        ...
        for (x = startX; x <= endX; x++) {
          ...
          if (cell != startCell) {
            ...
            tinymce.each(children, function(node) {
  ...
  function insertRow(before) {
    ...
    // Find first/last row
    tinymce.each(grid, function(row, y) {
      tinymce.each(row, function(cell) {
        if (isCellSelected(cell)) {
          cell = cell.elm;
  ...
  function insertCol(before) {
    ...
    tinymce.each(grid, function(row) {
      tinymce.each(row, function(cell, x) {
        if (isCellSelected(cell)) {
          posX = x;
  ...
  function deleteRows() {
    ...
    function deleteRow(tr) {
      ...
      tinymce.each(tr.cells, function(cell) {
      ...
      tinymce.each(grid[pos.y], function(cell) {
        var rowSpan;
    ...
    tinymce.each(rows.reverse(), function(tr) {
      deleteRow(tr);
    });
  ...
  function copyRows() {
    ...
    tinymce.each(rows, function(row, i) {
  ...
  function pasteRows(rows, before) {
    ...
    tinymce.each(rows, function(row) {
  ...
  function setEndCell(cell) {
    ...
    for (y = startY; y <= maxY; y++) {
      for (x = startX; x <= maxX; x++) {
        grid[y][x] && dom.addClass(grid[y][x].elm, 'mce-item-selected');
      }
    }
  }
