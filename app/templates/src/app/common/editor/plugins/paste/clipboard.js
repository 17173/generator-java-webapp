...
return function(editor) {
	...
	function createPasteBin() {
		...
		if (tinymce.Env.ie || tinymce.Env.gecko) {
	...
	function registerEventHandlers() {
		editor.on('keydown', function(e) {
			...
			if ((tinymce.util.VK.metaKeyPressed(e) && e.keyCode == 86) || (e.shiftKey && e.keyCode == 45)) {
				...
				if (tinymce.Env.ie && keyboardPastePlainTextState) {
					e.preventDefault();
		...
		editor.on('paste', function(e) {
			...
			if (tinymce.Env.ie && (!isKeyBoardPaste || e.ieFake)) {
				createPasteBin();
			...
			if (!editor.settings['paste_data_images']) {
				...
				while (i--) {
					...
					if (src && src.indexOf('data:image') === 0) {
						if (!nodes[i].attr('data-mce-placeholder') && src !== tinymce.Env.transparentSrc) {
							nodes[i].remove();
						}
					}
				}
