...
define("tinymce/Formatter", [
	...
	return function(ed) {
		...
		function apply(name, vars, node) {
			...
			function setElementFormat(elm, fmt, curSelNode) {
				fmt = fmt || format;

				if (elm) {
					if (fmt.onformat) {
						// copy fmt, avoid modified
						var newFmt = {};
						for (var i in fmt) {
							newFmt[i] = fmt[i];
						}
						fmt = newFmt;
						fmt.onformat(elm, fmt, vars, node, curSelNode);
					}
			...
			function applyRngStyle(rng, bookmark, node_specific, curSelNode) {
				var newWrappers = [], wrapName, wrapElm, contentEditable = true;

				// Setup wrapper element
				wrapName = format.inline || format.block;
				wrapElm = dom.create(wrapName);
				setElementFormat(wrapElm, null, curSelNode);
				...
				rangeUtils.walk(rng, function(nodes) {
					...
					function process(node) {
						...
						if (contentEditable && !hasContentEditableState && format.block &&
							...
							setElementFormat(node, null, curSelNode);
						...
						if (format.selector) {
							...
							each(formatList, function(format) {
								...
								if (dom.is(node, format.selector) && !isCaretNode(node)) {
									setElementFormat(node, format, curSelNode);
									found = true;
								}
							});
				...
				each(newWrappers, function(node) {
					...
					function mergeStyles(node) {
						...
						if (child && !isBookmarkNode(child) && matchName(child, format)) {
							...
							setElementFormat(clone, null, curSelNode);
			...
			if (format) {
				...
				curSelNode = ed.selection.getNode();
				...
				if (node) {
					if (node.nodeType) {
						...
						rng.setStartBefore(node);
						rng.setEndAfter(node);
						applyRngStyle(expandRng(rng, formatList), null, true, curSelNode);
					} else {
						applyRngStyle(node, null, true, curSelNode);
					}
				} else {
					if (!isCollapsed || !format.inline || dom.select('td.mce-item-selected,th.mce-item-selected').length) {
						...
						applyRngStyle(expandRng(selection.getRng(TRUE), formatList), bookmark, false, curSelNode);
